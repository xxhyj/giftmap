...
**핵심 흐름** — Home → Gift Finder(5단계) → Analyzing → Result → Product Detail
...
**찜 / 최근 본 상품** — 카드에서 찜하면 보관함 "찜한 상품"에 모이고, 해제하면 비워진다.
상세를 열면 "최근 본 상품"에 쌓인다

**보관함** — 세 세그먼트(찜한 상품·최근 본 상품·추천 기록) 각각의 빈 상태와 목록
